...
import { FaBus, FaClock, FaCheckCircle, FaUser, FaChair } from 'react-icons/fa';
...
function BusDetails() {
  ...
  const { defaultSource, defaultDestination } = location.state || {};
  ...
  useEffect(() => {
    if (selectedRoute) {
      const stops = [selectedRoute.source?.name, ...(selectedRoute.stops || []), selectedRoute.destination?.name].filter(Boolean);
      
      let initialSource = stops[0];
      let initialDest = stops[stops.length - 1];

      if (defaultSource && stops.includes(defaultSource)) initialSource = defaultSource;
      if (defaultDestination && stops.includes(defaultDestination)) initialDest = defaultDestination;

      setBoardingPoint(initialSource);
      setDroppingPoint(initialDest);
    }
  }, [selectedRoute, defaultSource, defaultDestination]);
        ...
        <div style={{ background: '#fff', padding: '20px 30px', borderRadius: '8px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <select
                ...
                style={{ display: 'block', padding: '10px', marginTop: '5px', borderRadius: '6px', border: '1px solid #ccc', outline: 'none', background: '#f9f9f9', minWidth: '250px', marginBottom: '10px' }}
              >
              ...
              {selectedRoute && (
                <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '10px' }}>
                  <div style={{ textAlign: 'left' }}>
                    <label style={{ fontSize: '12px', color: '#666', fontWeight: 'bold' }}>Boarding Point:</label>
                    <select
                      value={boardingPoint}
                      onChange={(e) => setBoardingPoint(e.target.value)}
                      style={{ display: 'block', padding: '8px', marginTop: '3px', borderRadius: '4px', border: '1px solid #ccc', outline: 'none', minWidth: '150px' }}
                    >
                      {[selectedRoute.source?.name, ...(selectedRoute.stops || []), selectedRoute.destination?.name].filter(Boolean).map((stop, idx) => (
                         <option key={'board-'+idx} value={stop}>{stop}</option>
                      ))}
                    </select>
                  </div>
                  <div style={{ textAlign: 'left' }}>
                    <label style={{ fontSize: '12px', color: '#666', fontWeight: 'bold' }}>Dropping Point:</label>
                    <select
                      value={droppingPoint}
                    ...
                    >
                      {[selectedRoute.source?.name, ...(selectedRoute.stops || []), selectedRoute.destination?.name].filter(Boolean).map((stop, idx) => (
                         <option key={'drop-'+idx} value={stop}>{stop}</option>
                      ))}
                    </select>
                  </div>
                </div>
